...
from tensorflow.keras.layers import BatchNormalization, Convolution1D, Dropout, Concatenate, Activation
from tensorflow.keras.regularizers import l2
...
def sine_cosine_to_angle(sine_cosine):
    ...
    for i in range(0, sine_cosine.shape[2], 2): # Iterate over sine and cosine pairs
        sine = sine_cosine[:, :, i]
        cosine = sine_cosine[:, :, i + 1]
        angle = np.arctan2(sine, cosine)
        predicted_angles[:, :, i//2] = np.degrees(angle) # Store the angles in degrees
...
def periodic_mae_for_angle(true_angles, predicted_angles, angle_index):
    # Extract the specific angle from all sequences and nucleotides
    true_angle = true_angles[:, :, angle_index]
    predicted_angle = predicted_angles[:, :, angle_index]

    # Create a mask for non-padded (non-zero) true angles for the specific angle
    mask = true_angle != 0

    # Apply the mask to filter out padded angles
    filtered_true_angle = true_angle[mask]
    filtered_predicted_angle = predicted_angle[mask]

    # Calculate the absolute difference between filtered true and predicted angles for the specified angle
    abs_diff = np.abs(filtered_true_angle - filtered_predicted_angle)

    # Adjust for the periodic nature of angles
    adjusted_diff = np.minimum(abs_diff, 360 - abs_diff)

    # Compute the mean of these adjusted differences for the specified angle
...
def periodic_mae(true_angles, predicted_angles):
    # Create a mask for non-padded (non-zero) true angles
    mask = true_angles != 0
    #print(mask)
    ...
    # Calculate the absolute difference between filtered true and predicted angles
    abs_diff = np.abs(filtered_true_angles - filtered_predicted_angles)

    # Calculate the complementary angle for each difference
    complementary_diff = 360 - abs_diff

    # Calculate the minimum of the absolute difference and its complementary angle
    min_diff = np.minimum(abs_diff, complementary_diff)

    # Compute the mean of these minimum differences
...
def inceptionBlock(x):
    ...
    # First path with 1x1 convolution
    ...
    # Second path with 1x1 then 3x3 convolution
    ...
    # Third path with 1x1 then 5x5 convolution
    ...
    # Fourth path for capturing higher order features
...
def deep3iBLock_with_attention(x, attention_mask, pos_ids=None):
    # First path - inception block followed by dilated CNN and attention
    ...
    # Second path - inception block followed by dilated CNN and attention
    ...
    # Third path - an average pooling followed by convolution and attention
    ...
    # Upsample to match the dimensions
    ...
    # Concatenate the output of all three paths
...
def custom_loss_SAINT(y_true, y_pred):
    ...
    #  padding results in sin(0)=0 and cos(0)=1, and sine values are at even indices [0, 2, 4, ...]
    # Construct a mask from the sine values (even indices)
    sine_values = y_true[:, :, ::2]  # Extract sine values , they are at even indices
    mask = tf.cast(tf.not_equal(sine_values, 0), tf.float32)  # 1 for non-padded, 0 for padded

    # Expand mask for all sine and cosine components
    ...
    masked_mse = tf.reduce_sum(squared_diff) / tf.reduce_sum(mask)  # Normalized by the number of non-padded elements

    return masked_mse



#num_sequences = NUM_OF_RNAs+1
...
sequence_length = MAXIMUM_LENGTH   # Length of each sequence
num_angles = 9         # Number of torsional angles per nucleotide
nucleotides = ['A', 'U', 'G', 'C']


model = get_model(num_features= 1284  )  #  772 , 1284
model.compile(optimizer=Adam(learning_rate=1e-3), loss=custom_loss_SAINT) # CUSTOM LOSS
...
X_train, X_val, y_train, y_val, attn_mask_train, attn_mask_val, pos_ids_train, pos_ids_val = train_test_split(
    features, sine_cosine_values, attention_masks, pos_ids, test_size=0.2, random_state=0
)
...
print(f"Periodic Mean Absolute Error: {periodic_mae_value} degrees")

# INDIVIDUAL MAE OF ANGLES

for i in range(9):
  angle_index = i  # Index for the specific angle you're interested in
  mae_for_angle = periodic_mae_for_angle(true_angles_degrees, predicted_angles_degrees, angle_index)

  print(f"Periodic Mean Absolute Error for Angle {angle_index + 1}: {mae_for_angle} degrees")
...
num_sequences = NUM_OF_RNAs+1  # Number of sequences
sequence_length = MAXIMUM_LENGTH   # Length of each sequence
num_angles = 9         # Number of torsional angles per nucleotide
nucleotides = ['A', 'U', 'G', 'C']


# Generate dummy attention masks (all ones, indicating all positions are valid)
attention_masks = np.ones((num_sequences, sequence_length))

# Generate positional identifiers (0 to sequence_length-1 for each sequence)
pos_ids = np.tile(np.arange(sequence_length), (num_sequences, 1))

predictions = model.predict([features, attention_masks, pos_ids])
predicted_angles_degrees = sine_cosine_to_angle(predictions)
